...
static void outlier_replace(int *bin, int i, Quantile q, double multpl,FILE *outlier_report);
...
static void outlier_replace(int *bin, int i, Quantile q, double multpl,FILE *outlier_report)
        { if(bin[2*i+1]>(q->cnt_node->num_reads)*multpl)
                { if(outlier_report!=NULL) fprintf(outlier_report,"%d\t%d\t%d\n",bin[2*i],bin[2*i+1],(q->cnt_node->num_reads));
...
void singularity_rm(int *bin, int nbins, int win_size, double p, double multple,FILE *outlier_report)
	{ int i,j,w_left,w_right;
	  ...
	  outlier_replace(bin,i,q,multple,outlier_report);
	  for(i=1;i<nbins;i++)
